...
void lv_port_indev_init(void)
{
    ...
    // keypad_init(); //按键已在程序启动时初始化

    /*Register a keypad input device*/
    indev_keypad = lv_indev_create();
    lv_indev_set_type(indev_keypad, LV_INDEV_TYPE_KEYPAD);
    lv_indev_set_read_cb(indev_keypad, keypad_read);
    lvgl_key_queue = Key_Event_Subscribe(key_handle);
...
static void keypad_read(lv_indev_t * indev_drv, lv_indev_data_t * data)
{
    ...
    xQueueReceive(lvgl_key_queue, &key_event, 0);
    uint8_t act_key = key_event.key_type;
    if (act_key == KEY_UNKOWN)
    {
        data->state = LV_INDEV_STATE_RELEASED;
        return;
    }
    ...
    if (key_event.key_action == KEY_RELEASE)
    {
        data->key = act_key;
        data->state = LV_INDEV_STATE_RELEASED;
        return;
    }
    if (key_event.key_action == KEY_PRESS)
    {
        data->key = act_key;
        data->state = LV_INDEV_STATE_PRESSED;
        return;
